...
const MyLiveAudioVisualizer: (props: Props) => ReactElement = ({
    mediaRecorder,
    width = '100%',
    height = '100%',
    barWidth = 2,
    ...
    recordingBarColor = 'rgb(190, 38, 38)',
    fftSize = 1024,
    maxDecibels = -10,
    minDecibels = -90,
    ...
    useEffect(() => {
        if (analyser && (mediaRecorder.state === 'recording' || mediaRecorder.state === 'paused')) {
            report();
        }
    }, [analyser, mediaRecorder.state]);

    const isClosingAudioContextRef = useRef(false);
    ...
    const report = useCallback(() => {
        ...
        if (mediaRecorder.state === 'recording' || mediaRecorder.state === 'paused') {
            ...
            processFrequencyData(data, mediaRecorder.state === 'recording');
            setTimeout(() => {
                requestAnimationFrame(report);
            }, 0);
        } else if (mediaRecorder.state === 'inactive' && context.state !== 'closed' && !isClosingAudioContextRef.current) {
            isClosingAudioContextRef.current = true;
            context.close();
        }
    }, [analyser, context.state]);

    const processFrequencyData = (data: Uint8Array, isRecording: boolean): void => {
        if (!canvasRef.current) return;

        const dataPoints = calculateBarData(data, canvasRef.current.width, barWidth, gap);
        draw(dataPoints, canvasRef.current, barWidth, gap, backgroundColor, isRecording ? recordingBarColor : barColor);
    };
